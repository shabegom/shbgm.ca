...
client.getEntries().then(response => {
  ...
  module.exports.insights = response.items
    .filter(item => item.sys.contentType.sys.id === "insights")
    .map(item => {
      items = [];
      item.fields.point.forEach(p => {
        item.fields.pointDescription.forEach(pD => {
